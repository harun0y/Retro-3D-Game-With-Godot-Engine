...
enum STATES {IDLE, CHASE, ATTACK, DEAD, HEALING}
...
func process_state_healing(delta):
	...
	if attack_counter < 15:
		...
		regen = 10
	health_manager.cur_health += delta * regen
	anim_player.play("healing_loop")
	if canSpawn:
		$TomatSpawner.spawnTomats()
		canSpawn = false
	$HealingParticles.emitting = true
	if health_manager.cur_health >= 90:
		current_state = STATES.CHASE
		$HealingParticles.emitting = false
...
func start_attack():
	can_attack = false
	if fmod(attack_counter, 4) == 0:
		anim_player.play("misille")
		attack_timer.wait_time = anim_player.get_animation("misille").length
	else:
		anim_player.play("attack")
		attack_timer.wait_time = anim_player.get_animation("attack").length
	attack_timer.start()
	attack_counter += 1
